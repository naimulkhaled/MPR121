 ...
 * Version: 1.0
 *   
 * Copyright 2015 Naimul Khaled <naimul.k@gmail.com>
 * Based on previous work by:
 *    Waiman Zhao <Binpower@foxmail.com> 
...
#else
     ReturnData[i] = Wire.receive(); // receive a byte as character
#endif
     i++;
   }
}
...
// Function to detect multiple sensors simultaneously, returns data as a bit for each sensor used
// Input variable sensors determines number of sensors to detect, maximum 12
short MPR121::getAll(uint8_t sensors)
{  
  if(sensors > 12)  sensors = 12;
  short returnVal = 0;
  int key = touch();
  int t;
